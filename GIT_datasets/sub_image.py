...
def main():
    ...
    Usage:
        ...
        The COCP-SR and SIM-SR datasets each require processing three folders.
        train/HR
        train/LR/X2
        train/LR/X4
    ...
    for root_path in [COCP_path,SIM_path]:
        ...
        opt['input_folder'] = root_path + 'LR/X2'
        opt['save_folder'] = root_path + 'LR/X2_sub'
        opt['scale'] = 2
        opt['wh'] = (848, 240)
        opt['crop_size'] = 128
        opt['step_h'] = 56
        ...
        opt['save_folder'] = root_path + 'LR/X4_sub'
        opt['scale'] = 4
        opt['wh'] = (424, 120)
        opt['crop_size'] = 64
        opt['step_h'] = 28
...
def worker(path, opt):
    ...
    h, w = img.shape[0:2]
    h_space = np.arange(0, h - crop_size + 1, step_h)
    if h - (h_space[-1] + crop_size) > thresh_size:
        h_space = np.append(h_space, h - crop_size)
    w_space = np.arange(0, w - crop_size + 1, step_w)
