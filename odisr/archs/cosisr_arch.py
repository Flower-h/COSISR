# Modified from https://github.com/Fanghua-Yu/OSRT
...
class DCNv4Pack(DCNv4):
    def forward(self, input, condition, shape=None):
        """
        :param query                       (N, H, W, C)
        ...
        if self.dw_kernel_size is not None:
            offset_mask_input = self.offset_mask_dw(condition.view(N, H, W, C).permute(0, 3, 1, 2))
            offset_mask_input = offset_mask_input.permute(0, 2, 3, 1).view(N, L, C)
        else:
            offset_mask_input = condition
...
class WindowAttention(nn.Module):
    ...
    def forward(self, xq, xkv=None, mask=None):
        """
        Args:
            xq: input features with shape of (num_windows*b, n, c)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if use_vit_condition:
            ...
            if vit_condition_type == '1conv':
                self.offset_conv = nn.Sequential(nn.Conv2d(condition_dim, 2, kernel_size=1, stride=1, padding=0, bias=True))
    ...
    def forward(self, x, x_size, condition):
        ...
        if self.use_vit_condition:
            ...
            offset = self.offset_conv(_condition).permute(0,2,3,1) + condition.permute(0,2,3,1)
...
class BasicLayer(nn.Module):
    """ A basic Swin Transformer layer for one stage.一组STL层，含一个RSTB模块中的所有STL层
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resolution.
...
class RSTB(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv = nn.Sequential(nn.Conv2d(dim, c_dim, 3, 1, 1))
        ...
        if self.use_dcn_conv:
            ...
            # self.conv_before_dcn = nn.Sequential(nn.Conv2d(dim, c_dim, 1, 1, 0, bias=True),
            #                                      nn.LeakyReLU(negative_slope=0.2, inplace=True))
            # self.dcn = DCNv2Pack(dim, dim, 3, padding=1)
            self.dcn = DCNv4Pack(c_dim, 3, 1, 1, 1, 16, 2.0, 3)
            self.conv_after_dcn = nn.Sequential(nn.Conv2d(c_dim, dim, 1, 1, 0, bias=True),
                                                nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward(self, x, x_size, condition):
        if self.use_dcn_conv:
            _x = self.conv(self.patch_unembed(self.residual_group(x, x_size, condition), x_size))#residual_group()是n个STL层操作 [N,c_dim,H,W]
            N, C, H, W = _x.shape
            L = H*W
            _x = _x.permute(0, 2, 3, 1).view(N, L, C)  #[N, L, C]
            _condition = self.offset_conv(condition).permute(0, 2, 3, 1).view(N, L, C)
            return self.patch_embed(self.conv_after_dcn(self.dcn(_x, _condition, (H, W)).view(N, H, W, C).permute(0, 3, 1, 2))) + x  #这里的dcn()是RSTB模块中最后一层的DACB操作 [N,em_dim,H,W]
        else:
...
class COSISR(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            self.conv_after_body = nn.Sequential(nn.Conv2d(embed_dim, c_dim, 3, 1, 1))
            # self.conv_after_body = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
        # elif resi_connection == 'SCconv':
        #     self.conv_after_body = nn.Sequential(ScConv(embed_dim))
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, c_dim, 3, 1, 1))
        elif resi_connection == '0conv':
        ...
        if self.use_dcn_conv:
            ...
            # self.dcn = DCNv2Pack(dim, dim, 3, padding=1)
            self.dcn = DCNv4Pack(c_dim, 3, 1, 1, 1, 16, 2.0, 3)
            self.conv_after_dcn = nn.Sequential(nn.Conv2d(c_dim, embed_dim, 1, 1, 0, bias=True),
                                                nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward(self, x, condition):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)  #[N,em_dim,H,W]
            if self.use_dcn_conv:
                _x = self.conv_after_body(self.forward_features(x, condition)) #[N,c_dim,H,W]
                N, C, H, W= _x.shape
                L = H*W
                _x = _x.permute(0, 2, 3, 1).view(N, L, C)#[N, L, C]
                _condition = self.offset_conv(condition).permute(0, 2, 3, 1).view(N, L, C)  #[N, L, C]
                x = self.conv_after_dcn(self.dcn(_x, _condition, (H, W)).view(N, H, W, C).permute(0, 3, 1, 2)) + x  #[N,em_dim,H,W]
            else:
...
if __name__ == '__main__':
    ...
    c = torch.randn((1, 2, 64, 64)).to(device)
    x = model(x, c)
    print(x.shape)
