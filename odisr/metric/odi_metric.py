...
def calculate_psnr_ws(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    img_w = compute_map_ws(img)#改

    mse = np.mean(np.multiply((img - img2)**2, img_w))/np.mean(img_w)#改
    if mse == 0:
        return float('inf')
...
def calculate_ssim_ws(img, img2, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    for i in range(img.shape[2]):
        ssims.append(_ws_ssim(img[..., i], img2[..., i]))  #改
...
def genCOI(h):#改
    ...
    y = (m0 - h) * dx
    p = (L ** 2 + y ** 2)**0.5
    w = f ** 2 * b ** 4 * L * (a_c * p - ac * y) / ((ac * p - a_c * y) ** 3 * p)

    return w

def compute_map_ws(img):#改
